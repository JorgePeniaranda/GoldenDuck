...
export default function ButtonWithPopover({ PopoverIcon, children, arialLabel }: Props) {
  return (
    <Popover className={style.Popover} aria-label={arialLabel}>
      <Popover.Button className={style.PopoverButton}>{PopoverIcon}</Popover.Button>
        ...
        >
          ...
          <Popover.Panel className={style.PopoverPanel} static>{children}</Popover.Panel>
        </Transition>
    </Popover>
  )
}
